...
// CyberSessionBlockKey preserves the original explicit-session API used by
// older audit hooks while routing through the current key derivation logic.
...
// IsCyberSessionBlocked is a compatibility shim for the pre-transcript audit
// hook. New callers should use FindCyberSessionBlockedForRequest.
...
// MarkCyberSessionBlockedLegacy preserves the old single-key behavior for
// callers that cannot provide a request scope.
func (s *OpenAIGatewayService) MarkCyberSessionBlockedLegacy(ctx context.Context, key string) {
	s.MarkCyberSessionBlocked(ctx, "", []string{key})
}
