...
func TestContentModerationCheck_PreHashBlocksWhenFlaggedPromptRemainsInHistory(t *testing.T) {
	cfg := defaultContentModerationConfig()
	cfg.Enabled = true
	cfg.PreHashCheckEnabled = true
	cfg.APIKeys = []string{"sk-test"}
	cfg.BlockStatus = http.StatusConflict
	cfg.BlockMessage = "historical risk input"
	rawCfg, err := json.Marshal(cfg)
	require.NoError(t, err)

	flagged := ContentModerationInput{Text: "blocked prompt"}
	flagged.Normalize()
	hashCache := &contentModerationTestHashCache{hashes: map[string]struct{}{
		flagged.Hash(): {},
	}}
	...
	require.True(t, decision.Blocked)
	require.Equal(t, ContentModerationActionHashBlock, decision.Action)
	require.Equal(t, flagged.Hash(), decision.InputHash)
	require.Contains(t, decision.Message, flagged.Hash())

	logs := requireContentModerationLogCount(t, repo, 1)
	require.Equal(t, "blocked prompt", logs[0].InputExcerpt)
}

func TestContentModerationCheck_PreHashBlocksRepeatedContinueWhenFlaggedPromptRemainsInHistory(t *testing.T) {
	cfg := defaultContentModerationConfig()
	cfg.Enabled = true
	cfg.PreHashCheckEnabled = true
	cfg.APIKeys = []string{"sk-test"}
...
func TestContentModerationCheck_PreHashAllowsCleanConversationAfterFlaggedPrompt(t *testing.T) {
	cfg := defaultContentModerationConfig()
	cfg.Enabled = true
	cfg.Mode = ContentModerationModePreBlock
	cfg.PreHashCheckEnabled = true
	...
	flagged := ContentModerationInput{Text: "blocked prompt"}
	flagged.Normalize()
	clean := ContentModerationInput{Text: "what is the weather today?"}
	clean.Normalize()
	hashCache := &contentModerationTestHashCache{hashes: map[string]struct{}{
		flagged.Hash(): {},
	}}
	...
	require.False(t, decision.Blocked)
	require.Equal(t, ContentModerationActionAllow, decision.Action)
	require.Equal(t, []string{clean.Hash()}, hashCache.snapshotChecked())
}
