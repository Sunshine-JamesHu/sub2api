export default {
  ...
  admin: {
    ...
    riskControl: {
      ...
      blockedKeywordsPlaceholder: '每行输入一条拦截规则，例如：\n敏感词1\n哈哈哈 A CCB',
      blockedKeywordsDescription: '匹配忽略大小写；同一行包含多个空格分隔词时需全部命中，不同行任意命中即可拦截；命中后会按下方策略决定是否调用上游审计接口。',
      ...
      blockedKeywordsLimit: '最多保存 {max} 条规则，单条长度不超过 200 个字符；重复项会自动去重。',
